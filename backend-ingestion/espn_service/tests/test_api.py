...
@pytest.mark.django_db
class TestPublicSyncEndpoints:
    """Public sync read API (no auth)."""

    def test_sync_teams_returns_results(self, api_client: APIClient, team: Team):
        response = api_client.get("/api/sync/v1/teams", {"limit": 10})
        assert response.status_code == status.HTTP_200_OK
        body = response.json()
        assert "results" in body
        assert body["has_more"] is False
        assert len(body["results"]) >= 1
        assert body["results"][0]["espn_id"] == "1"
        assert body["results"][0]["logo"] == "https://example.com/logo.png"
        assert isinstance(body["results"][0]["logos"], list)
        assert body["results"][0]["logos"]

    def test_sync_teams_invalid_cursor(self, api_client: APIClient):
        response = api_client.get(
            "/api/sync/v1/teams",
            {"after_updated_at": "not-a-date", "after_id": "1"},
        )
        assert response.status_code == status.HTTP_400_BAD_REQUEST
