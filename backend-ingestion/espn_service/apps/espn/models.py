...
class Team(TimestampMixin):
    ...
    # Bumps only when ingest detects a semantic change (watermark / public sync API).
    sync_cursor_at = models.DateTimeField(default=timezone.now, db_index=True)
    ...
    def primary_logo(self) -> str | None:
        """Best-effort display URL from ``logos`` JSON (handles ESPN's mixed shapes)."""
        normalized = normalize_espn_logos(self.logos)
        return primary_logo_href(normalized)
...
class AllowlistedCompetition(TimestampMixin):
    """Product allowlist driving Celery horizon + team jobs."""

    TEAM_SOURCE_LEAGUE = "league_roster"
    TEAM_SOURCE_TOURNAMENT = "tournament_roster"
    TEAM_SOURCE_CHOICES = [
        (TEAM_SOURCE_LEAGUE, "League roster"),
        (TEAM_SOURCE_TOURNAMENT, "Tournament roster"),
    ]

    sport_slug = models.CharField(max_length=50, db_index=True)
    league_slug = models.CharField(max_length=80, db_index=True)
    team_source = models.CharField(max_length=32, choices=TEAM_SOURCE_CHOICES)
    display_name = models.CharField(max_length=200)
    enabled = models.BooleanField(default=True)
    # Last calendar date (inclusive) successfully fetched for horizon extension.
    extend_cursor_date = models.DateField(null=True, blank=True)
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["sport_slug", "league_slug"],
                name="uniq_allowlist_sport_league",
            ),
        ]

    def __str__(self) -> str:
        return f"{self.display_name} ({self.sport_slug}/{self.league_slug})"
